...
public class AvatarController {
    ...
    //public Long addAvatar (@RequestParam MultipartFile file,
                       //    @RequestParam Long studentId) throws IOException {
        //return avatarService.addAvatar(file, studentId);
    //}

    @PostMapping(value = "/{id}/avatar" , consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<String> uploadAvatar (@PathVariable Long id, @RequestParam MultipartFile avatar) throws IOException{
        if (avatar.getSize() >= 1024 * 300) {
            return ResponseEntity.badRequest().body("Рамер файла превышен");
        }
